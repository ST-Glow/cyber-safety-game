...
func _load_web_configuration() -> void:
	if not OS.has_feature("web"):
		...
		return
	var raw: Variant = JavaScriptBridge.eval("JSON.stringify({apiBaseUrl:(window.GODOT_AI_CONFIG&&window.GODOT_AI_CONFIG.apiBaseUrl)||'',ticket:(new URLSearchParams(window.location.search)).get('ticket')||''})")
	var parsed: Variant = JSON.parse_string(String(raw))
	if not parsed is Dictionary:
		_set_unconfigured("网页 AI 配置无法读取")
		return
	_apply_configuration(String(parsed.get("apiBaseUrl", "")), String(parsed.get("ticket", "")))


func _apply_configuration(api_base_url: String, ticket: String) -> void:
	_api_base_url = api_base_url.strip_edges().trim_suffix("/")
	_ticket = ticket.strip_edges()
	if not _is_allowed_api_url(_api_base_url):
