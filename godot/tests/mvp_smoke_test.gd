...
func _run_test() -> void:
	...
	await process_frame
	_expect(manager.state == GameManager.GameState.QUIZ, "first correct answer opens the second formal question")
	game.call("_on_quiz_choice_selected", 0)
	await process_frame
	_expect(manager.state == GameManager.GameState.FINISHED, "second correct answer finishes the run")
	_expect(manager.quiz_attempts == 3, "two formal questions retain all three attempts")
	_expect(completed_result.get("score", -1) == 90, "one retry across two questions scores 90")
	_expect(completed_result.get("stars", -1) == 3, "score 90 awards three stars")
	_expect(completed_result.get("normalized_score", -1) == 90, "first-level normalized score matches its 100-point score")
